extends DetectComponentZombie
class_name DetectComponentGargantuar
...
func judge_is_have_enemy():
	...
	for ray_area in all_ray_area:
		...
		for enemy_area in all_enemy_area:
			## 检测到罐子敌人
			if enemy_area.owner is ScaryPot:
				print("检测到罐子")
				pot = enemy_area.owner
				signal_can_attack.emit()
				return true
			elif enemy_area.owner is BrainOnZombieMode:
				#print("检测到脑子")
